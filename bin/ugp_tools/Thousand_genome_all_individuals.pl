...
Description:
	Script to find all the fastq files found within each sequence_read directory found here:
    ...
    ** NOTE: estimated size of all data is ~100T.

	Directories will be created per individual and corresponding files will be correctly 
	placed.
...
Additional options:
	...
	--cpu, -c :	How many concurrent downloads to run at one time. [default 1].	

EOU

my %c_opts = ();
GetOptions( \%c_opts, "run|r", "cpu|c=i" );
unless ( $c_opts{run} ) {
    die $usage;
}
...
for my $indiv (@individuals) {
    ...
    my $ftpcmd = "wget ftp://ftp-trace.ncbi.nih.gov/1000genomes/"
