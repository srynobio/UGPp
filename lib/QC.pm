...
#TODO:
# Need full refactor of this package
...
sub fastqc_check {
    ...
    if (@fails) {
        $tape->WARN(
            "Fastqc reported contains FAIL files, please review QC-report.txt");
        chomp @fails;
        @reports = map { $_ } @fails;
    }
    ...
    foreach my $d (@data) {
        ...
        foreach my $result (<$DT>) {
            ...
            if (    $view[0] eq 'Encoding'
                and $view[1] ne 'Sanger / Illumina 1.9' )
            {
                $fail++;
                next;
            }
            ...
            if (    $view[0] eq 'Total Duplicate Percentage'
                and $view[1] > '60.0' )
            {
                $fail++;
                next;
